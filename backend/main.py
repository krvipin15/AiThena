...
async def process_youtube(youtube_url: str = Form(...), token: str = Form(...)):
    verify_firebase_token(token)
...
async def summarize(text: str = Form(...), token: str = Form(...)):
    verify_firebase_token(token)
    summary = summarize_text(text)
    return {"summary": summary}

@app.post("/store_result", response_model=StoreResultResponse)
async def store_result(user_id: str = Form(...), result: str = Form(...), token: str = Form(...)):
    verify_firebase_token(token)
...
async def generate_flashcards(text: str = Form(...), token: str = Form(...)):
    verify_firebase_token(token)
    flashcards = generate_flashcards_from_text(text)
    return {"flashcards": flashcards}

@app.post("/generate_mcq", response_model=MCQResponse)
async def generate_mcq(text: str = Form(...), token: str = Form(...)):
    verify_firebase_token(token)
